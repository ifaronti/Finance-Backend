...
export const Register:controller = async (req, res) => {
  const { email, name, password, income, avatar, expenses, balance }: reqBody =
    req.body;
  const salt = await bcryptjs.genSalt(10);
  const hashed = await bcryptjs.hash(password, salt);

  const user = await prisma.user.create({
    data: {email, income, avatar, name, password: hashed, expenses, balance}
  });

  if(!user.id){return res.end('User not created, please try again')}
